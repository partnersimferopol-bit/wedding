...
Оба способа делают одно и то же: сервер принимает заявки и шлёт их в беседу ВК.
...
## Б.4. Секреты (ключ ВК и беседа)

Worker → **Settings** → **Variables and Secrets** → **Add**:

| Тип | Имя | Значение |
|-----|-----|----------|
| Secret | `VK_GROUP_TOKEN` | новый ключ API сообщества ВК |
...
Проверка в браузере: откройте `https://ВАШ_URL/health` — должно быть `{"ok":true,"vk":true}`.
...
3. Тестовая заявка на сайте → сообщение в [беседе ВК](https://vk.com/im/convo/-202321163).
